...
class VoiceActivityDetector():
    ...
    def save_only_speech_file(self, input_file, output_file, speech_labels):
        ...
        with wave.open(input_file, 'rb') as input_wf:
            ...
            with wave.open(output_file, 'wb') as output_wf:
                ...
                # Đặt số frame ban đầu là 0, trước khi bắt đầu ghi dữ liệu
    ...
    def plot_saved_file(self, outputfile):
        # plot the speech labels
        data = self.data
        plt.figure()
        plt.plot(data)
...
if __name__ == "__main__":
    ...
    v = VoiceActivityDetector('test1.wav')
    raw_detection = v.detect_speech()
    speech_labels = v.convert_windows_to_readible_labels(raw_detection)
    print(speech_labels)
    # v.plot_detected_speech_regions()
    # crop the original sound file to get all the speeches alone then save it to a new file
    v.plot_readible_labels(speech_labels)
    v.save_only_speech_file('test1.wav', 'test1_output.wav', speech_labels)
